...
describe('API smoke tests', () => {
  ...
  it('records upgrade intent and returns pending status', async () => {
    const first = await api().post('/v1/upgrade-intents').send({ plan: 'plus', source: 'pricing' }).expect(201)
    expect(first.body.data?.plan).toBe('plus')
    expect(first.body.data?.alreadyPending).toBe(false)

    const second = await api().post('/v1/upgrade-intents').send({ plan: 'plus', source: 'pricing' }).expect(200)
